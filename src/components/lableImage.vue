...
async function saveLable() {
  ...
  if (!lable) throw new Error("lable is null");
...
async function loadLable(imageName: string) {
  ...
  content.split("\n").forEach((item) => {
    ...
    if (!lable) throw new Error("lable is null");
    const _lable = lable.getLabelById(Number(cid));
...
async function loadImageAction(isNext = true) {
  try {
    await saveLable();
    if (!fileIo) return;
    const imageData = isNext ? fileIo.nextImage : fileIo.prevImage;
    if (!imageData) return;
...
function drawAction() {
  ...
  function move(e: MouseEvent) {
    ...
    if (!isDraw || !canvasCtx || !lable || !canvasDom.value) return;
    refreshDraw();
    const color = lable.currentLabel.color;
  ...
  function end(e: MouseEvent) {
    ...
    isDraw = false;
    const id = getRadomId();
    const { id: cid, name, color } = lable.currentLabel;
...
async function init() {
  ...
  lable = new Lable();
  lable.setIndex(states.lableIndex);
...
function loadEvents() {
  ...
  window.addEventListener("keydown", (e) => {
    if (e.key === "d" || e.key === "D") {
      ...
      const _lable = lable?.prevLabel;
      Snackbar.info("切换到标签:" + _lable?.name);
      refreshDraw();
    } else if (e.key === "e" || e.key === "E") {
      lable?.nextLabel;
      Snackbar.info("切换到标签:" + lable?.currentLabel.name);
      refreshDraw();
...
<template>
  <div class="lableImageBody">
    <header>
      ...
      <var-button
        type="primary"
        @click="lable?.prevLabel"
        >上一个标签</var-button
      >
      <var-button
        type="primary"
        @click="lable?.nextLabel"
        >下一个标签</var-button
      >
    </header>
    <main>
